...
object LectureScheduleViewModel : ViewModel() {
    ...
    val _viewStart = MutableLiveData<Boolean>().apply {
        value = false
    }
    val viewStart: LiveData<Boolean> = _viewStart
    ...
    fun setIsViewLoading(status : Boolean){
        _isViewLoading.postValue(status)
    }

    fun setLectureSchedule(v : LectureSchedule?){
        _lectureSchedule.postValue(v)
    }

    fun loadSchedule(loginParam: LoginParam){
        _isViewLoading.postValue(true)
        myAPI.getSchedule(loginParam).enqueue(object  : Callback<LectureSchedule> {
